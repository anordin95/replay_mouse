...
from nmz_setup import PRAYER_POTS_FILENAME, RANGE_POTS_FILENAME, ABSORPTION_POTS_FILENAME, QUICK_PRAY_LOC_FILE
...
RANGE_LVL_SIP_AT = 5.0

# Drink 1 absorption pot every 200 seconds
# Based on observation of 1-monster hitting at a time.
SIP_ABSORTION_EVERY_X_SECONDS = 200.0
...
def double_click_quick_pray(location: ClickLocation):
	...
	action = ClickAction(click_spot_x, click_spot_y)
	...
	action = ClickAction(click_spot_x, click_spot_y)
	duration_min, duration_max = 10, 30
	duration = random.randint(duration_min, duration_max) / 1000.0
	action.duration = duration
	action.execute()
...
def is_done(finished_absorptions_time):
	...
	beginning_absorption_time -= 10 * 60
